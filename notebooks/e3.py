...
class CacheEnv(gym.Env):
    ...
    def nn_state(self):
        ...
        for k in self.pages:
            vals = self.pages[k]
            state_lu.append(vals[0]) #Flatten
            state_fu.append(vals[1]) #Flatten
            
        v_1 = self.normalize(np.array(state_lu))
        v_2 = self.normalize(np.array(state_fu))
        return np.concatenate((v_1,v_2),axis=0)
